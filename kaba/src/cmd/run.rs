...
use runtime::runtime::{stream::RuntimeStream, Runtime};
...
pub fn handle(file_path: &Path) {
    let src = compiler::normalize_newlines(&exit_on_error!(read_content(file_path)));

    let compilation = compiler::compile(&src).map_err(|e| Error::CompilationError {
        path: Some(file_path),
        src: &src,
        message: e.message,
        span: e.span,
